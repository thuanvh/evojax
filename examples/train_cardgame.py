...
def main(config):
    log_dir = './log/card_game'
    if not os.path.exists(log_dir):
        os.makedirs(log_dir, exist_ok=True)
    logger = util.create_logger(
        name='CardGame', log_dir=log_dir, debug=config.debug)
    logger.info('EvoJAX CardGame')
    logger.info('=' * 30)

    max_steps = 500
    train_task = CardGame(test=False, max_steps=max_steps)
    test_task = CardGame(test=True, max_steps=max_steps)
    policy = MLPPolicy(
