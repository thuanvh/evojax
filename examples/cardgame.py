...
def create_bubbles(key: jnp.ndarray, is_agent: bool) -> BubbleStatus:
    ...
    cards_values = []
    for i in range(CARD_TOTAL_CARD):
        cards_values.append(random.uniform(CARD_MAX_ATTACK))
        cards_values.append(CARD_MAX_BLOOD)
        if i < CARD_INIT_OPEN :
            cards_values.append(1) # OPEN
        else :
            cards_values.append(0) # CLOSE
    cards_jnp = jnp.array(cards_values)
    return BubbleStatus(
        ...
        valid=1, poison_cnt=0, cards = cards_jnp)
...
def get_obs(agent: BubbleStatus,
    ...
    item_cards = []
    for i in range(CARD_TOTAL_CARD):
        opened = items.cards[i*3+2] == 1
        item_cards.append(items.cards[i*3] if opened else -1)
        item_cards.append(items.cards[i*3+1] if opened else -1)
        item_cards.append(items.cards[i*3+2])
    item_cards_jnp = jnp.array(item_cards).ravel()
    #return jnp.concatenate([sensor_obs.ravel(), vel_xy], axis=0)
    return jnp.concatenate([agent.cards.ravel(), item_cards_jnp], axis=0)


class CardGame(VectorizedTask):
    """Water world.
    ref: https://cs.stanford.edu/people/karpathy/reinforcejs/waterworld.html
    """
